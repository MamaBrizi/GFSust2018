  ...
  class SideNav {
    ...
    translateSidenavX(touchX) {
      ...
      if (this.options.edge === 'left') {
        ...
        const isRightDirection = touchX >= this.options.menuWidth / MENU_WIDTH_HALF;
        this.menuOut = isRightDirection;

        this.$menu.css('transform', `translateX(${touchX - this.options.menuWidth}px)`);
      } else {

        const isLeftDirection = touchX < window.innerWidth - this.options.menuWidth / MENU_WIDTH_HALF;
        this.menuOut = isLeftDirection;

        let rightPos = touchX - this.options.menuWidth / MENU_WIDTH_HALF;
        if (rightPos < 0) {
          rightPos = 0;
        }
